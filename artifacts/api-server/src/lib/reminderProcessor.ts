...
— DueRadar`.trim();
    ...
    const activeWithRuleCounts = await db
      ...
      .from(obligationsTable)
      .leftJoin(reminderRulesTable, eq(reminderRulesTable.obligationId, obligationsTable.id))
      .where(eq(obligationsTable.status, "active"))
      .groupBy(
        obligationsTable.id,
    ...
    for (const row of activeWithRuleCounts) {
      const { score } = computeHealthScore({
        ...
        backupOwnerEmail: row.backupOwnerEmail ?? null,
        activeReminderRuleCount: row.activeRuleCount ?? 0,
      });
      await db.update(obligationsTable).set({ healthScore: score }).where(eq(obligationsTable.id, row.id));
    }
